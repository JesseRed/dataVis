...
get_currently_selected_data<-function(data, g1, g2, t1, t2, f){
...
# d$color1 ... col ........ die Color palette die zu den Werten passen
  ...
  if ((! t1 == t2) && (!g1==g2)){
    d$string1 = paste0("Compare the in group diff of ", t1," vs ", t2, "between groups", "\n",
                       "unpaired t-test\n")
    d$data1 = get_data_group_trial_freqmean(g_data(),g1, t1, freq())-get_data_group_trial_freqmean(g_data(),g1, t2, freq())
    d$data2 = get_data_group_trial_freqmean(g_data(),g2, t1, freq())-get_data_group_trial_freqmean(g_data(),g2, t2, freq())
    d$my_paired = FALSE

  }else{
    d$data1 = get_data_group_trial_freqmean(g_data(),g1, t1, freq())
    d$data2 = get_data_group_trial_freqmean(g_data(),g2, t2, freq())
    if (t1 == t2) {
...
get_data_freqmean <- function(data, freq, sel_freqs= g_sel_freqs(), method = g_act_method()){
  data_freq = filter_by_selfreq(data, freq, sel_freqs, method)
  data_freqmean = get_freqmean(data_freq, freq, sel_freqs = sel_freqs, method = method)
...
get_data_groupmean_trial <- function(data, group,trial, sel_freqs= g_sel_freqs(), method = g_act_method()){
...
get_data_groupmean_trial_freq <- function(data, group,trial,freq, sel_freqs= g_sel_freqs(), method = g_act_method()){
  #cat(file = stderr(), paste0("get_data_groupmean_trial_freq dim(data)=",dim(data),"\n"))
  data_groupmean_trial = get_data_groupmean_trial(data, group,trial, sel_freqs = sel_freqs, method = method)
  #cat(file = stderr(), paste0("get_data_groupmean_trial_freq dim(data_groupmean_trial)=",dim(data_groupmean_trial),"\n"))
  data_groupmean_trial_freq = filter_by_selfreq(data_groupmean_trial, freq, sel_freqs = sel_freqs, method = method)
...
get_data_group_freq <- function(data, group, freq, sel_freqs= g_sel_freqs(), method = g_act_method()){
  #cat(file = stderr(), paste0("get_data_group_freq dim(data)=",dim(data),"\n"))
  data_group = get_data_group(data, group)
  #cat(file = stderr(), paste0("dim(data_group)=",dim(data_group),"\n"))

  data_group_freq = filter_by_selfreq(data_group, freq, sel_freqs = sel_freqs, method = method)
...
filter_by_selfreq <- function(data, freq, sel_freqs = g_sel_freqs(), method = g_act_method()){
...
get_freqmean <- function(data, freq, sel_freqs = g_sel_freqs(), method = g_act_method()){
  num_dims = length(dim(data))
  if (method=="Transferentropy"){
    #Transferentropy has no frequencies
    #
...
get_data_group_freqmean <- function(data, group, freq, sel_freq = g_sel_freqs(), method = g_act_method()){
  ...
  data_group_freqmean = get_freqmean(data_group_freq, freq, sel_freqs = sel_freqs, method = method)
...
get_data_group_trial_freq <- function(data, group, trial, freq, sel_freqs= g_sel_freqs(), method = g_act_method()){
  data_group_trial = get_data_group_trial(data, group, trial)
#  x = (as.numeric(ufreq_list)>freq[1]) == (as.numeric(ufreq_list)<freq[2])
  data_group_trial_freq = filter_by_selfreq(data_group_trial, freq, sel_freqs = sel_freqs, method = method)
  # x = sel_freqs
  # if (length(dim(data))==4){
...
get_data_group_trial_freqmean <- function(data, group, trial, freq, sel_freqs= g_sel_freqs(), method = g_act_method()){
  #cat(file = stderr(), paste0("trial = ", trial, "\n"))
  data_group_trial = get_data_group_trial(data, group, trial)
  data_group_trial_freq = filter_by_selfreq(data_group_trial, freq,  sel_freqs = sel_freqs, method = method)
  data_group_trial_freqmean = get_freqmean(data_group_trial_freq, freq, sel_freqs = sel_freqs, method = method)
#  x = (as.numeric(ufreq_list)>freq[1]) == (as.numeric(ufreq_list)<freq[2])
  # x = sel_freqs
  # if (length(dim(data))==4){
  #   data_group_trial_freq = data_group_trial[ , , x]
...
get_data_group_region_trial_freq <- function(data, group, region, trial, freq, sel_freqs= g_sel_freqs(), method = g_act_method()){
...
#  x = (as.numeric(ufreq_list)>freq[1]) == (as.numeric(ufreq_list)<freq[2])
  data_group_region_trial_freq = filter_by_selfreq(data_group_region_trial, freq, sel_freqs = sel_freqs, method = method)
  #   x = sel_freqs
  # if (length(dim(data))==4){
