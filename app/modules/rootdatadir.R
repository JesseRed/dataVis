...
rootdatadirUI <- function(id){
  ...
  tagList(
    ...
    fluidRow(
      column(8, shinyDirButton(
        ns("dirbutton"),label = "select root data dir", title = "my title",
        buttonType = "default"),
      column(4,
             verbatimTextOutput("dir", placeholder = TRUE)
             )

#      fileInput(ns("data dir"), label = "data dir", accept = c(".csv", ".tsv"))),
...
rootdatadirServer <- function(id) {
  moduleServer(
    ...
    function(input, output, session) {
      ...
      #   input,
      #   "dirbutton",
      #   roots = c("G:\Programming\dataVISold\data"),
