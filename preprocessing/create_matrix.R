  ...
  for (n_region in uregion_list){
    ...
    for (n_trial in utrial_list){
      ...
      for (n_freq in ufreq_list){
        ...
        #print(col_name)
        #print(cat(sprintf("mdat[%d,%d,%d,%d]=%g ", num_subj, num_region, num_trial, num_freq,tmp)))
        #print("x")
...
saveRDS(utrial_list,file = 'utrial_list.Rda')
saveRDS(ufreq_list,file = 'ufreq_list.Rda')
